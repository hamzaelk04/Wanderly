<aside
    class="h-screen w-64 fixed left-0 top-0 bg-slate-50 flex flex-col py-8 justify-between z-50 font-['Plus_Jakarta_Sans'] font-medium text-sm">
    <div class="mb-8 px-2 flex items-center gap-3">
        <div class="w-10 h-10 rounded-xl bg-primary flex items-center justify-center text-white">
            <span class="material-symbols-outlined" data-icon="explore"
                style="font-variation-settings: 'FILL' 1;">explore</span>
        </div>
        <div>
            <h1 class="text-xl font-black text-sky-700 font-headline leading-tight">Organizer Hub</h1>
            <p class="text-xs font-medium text-slate-500">Wanderly Curator</p>
        </div>
    </div>
    <nav class="flex-1 flex flex-col gap-1">
        @if (auth()->user()->isAdmin())
        <a id="dashboard"
            class="flex items-center gap-3 p-3 text-slate-500 hover:bg-slate-200/50 hover:translate-x-1 font-['Plus_Jakarta_Sans'] text-sm font-medium transition-all duration-200 ease-in-out"
            href="/admin">
            <span class="material-symbols-outlined" data-icon="dashboard"
                style="font-variation-settings: 'FILL' 1;">dashboard</span>
            <span>Dashboard</span>
        </a>
        <a id="create-monument"
            class="flex items-center gap-3 p-3 text-slate-500 hover:bg-slate-200/50 hover:translate-x-1 font-['Plus_Jakarta_Sans'] text-sm font-medium transition-all duration-200 ease-in-out"
            href="/create/monument">
            <span class="material-symbols-outlined" data-icon="add_circle">add_circle</span>
            <span>Create Monument</span>
        </a>
        <a class="flex items-center gap-3 p-3 text-slate-500 hover:bg-slate-200/50 hover:translate-x-1 font-['Plus_Jakarta_Sans'] text-sm font-medium transition-all duration-200 ease-in-out"
            href="/admin/manage/monuments">
            <span class="material-symbols-outlined" data-icon="account_balance">account_balance</span>
            <span>Manage Monuments</span>
        </a>
        <a id="manage-event"
            class="flex items-center gap-3 p-3 text-slate-500 hover:bg-slate-200/50 hover:translate-x-1 font-['Plus_Jakarta_Sans'] text-sm font-medium transition-all duration-200 ease-in-out"
            href="/admin/manage/events">
            <span class="material-symbols-outlined" data-icon="event">event</span>
            <span>Manage Events</span>
        </a>
        <a id="create-category"
            class="flex items-center gap-3 p-3 text-slate-500 hover:bg-slate-200/50 hover:translate-x-1 font-['Plus_Jakarta_Sans'] text-sm font-medium transition-all duration-200 ease-in-out"
            href="/create/category">
            <span class="material-symbols-outlined" data-icon="add_circle">category</span>
            <span>Add Category</span>
        </a>
        @elseif (auth()->user()->isOrganizer())
        <a id="create-event"
            class="flex items-center gap-3 p-3 text-slate-500 hover:bg-slate-200/50 hover:translate-x-1 font-['Plus_Jakarta_Sans'] text-sm font-medium transition-all duration-200 ease-in-out"
            href="/create/event">
            <span class="material-symbols-outlined" data-icon="add_circle">add_circle</span>
            <span>Create Event</span>
        </a>
        <a id="manage-event"
            class="flex items-center gap-3 p-3 text-slate-500 hover:bg-slate-200/50 hover:translate-x-1 font-['Plus_Jakarta_Sans'] text-sm font-medium transition-all duration-200 ease-in-out"
            href="/organizer/manage/events">
            <span class="material-symbols-outlined" data-icon="event">event</span>
            <span>Manage Events</span>
        </a>
        @endif
        <!-- Dashboard Active -->
        <a class="flex items-center gap-3 p-3 text-slate-500 hover:bg-slate-200/50 hover:translate-x-1 font-['Plus_Jakarta_Sans'] text-sm font-medium transition-all duration-200 ease-in-out"
            href="#">
            <span class="material-symbols-outlined" data-icon="confirmation_number">confirmation_number</span>
            <span>Bookings</span>
        </a>
    </nav>
    <div class="pt-4 mt-auto border-t border-slate-200">
        <a class="flex items-center space-x-3 px-4 py-3 text-slate-500 hover:bg-slate-100 rounded-lg transition-all duration-300"
            href="#">
            <span class="material-symbols-outlined" data-icon="help">help</span>
            <span class="font-label">Help</span>
        </a>
        <a class="flex items-center space-x-3 px-4 py-3 text-slate-500 hover:bg-slate-100 rounded-lg transition-all duration-300"
            href="logout">
            <span class="material-symbols-outlined" data-icon="logout">logout</span>
            <span class="font-label">Logout</span>
        </a>
    </div>
</aside>