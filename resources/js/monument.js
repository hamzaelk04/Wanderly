document.addEventListener("DOMContentLoaded", () => {
    const STORAGE_KEY = "favorite_monuments";
    ...
    function updateUI(id) {
        const btns = document.querySelectorAll(`.favorite-btn[data-monument-id="${id}"]`);
    ...
    document.querySelectorAll(".favorite-btn").forEach(btn => {
        btn.addEventListener("click", () => {
            const id = btn.dataset.monumentId;
            toggleFavorite(id);
        });
    });
    ...
    document.querySelectorAll(".favorite-btn").forEach(btn => {
        updateUI(btn.dataset.monumentId);
    });
    ...
    function renderFavorites() {
        ...
        const favoriteCards = Array.from(cards).filter(card =>
            favorites.includes(card.dataset.monumentId)
        );
        ...
        favoriteCards.forEach(card => {
            const id = card.dataset.monumentId;
